...
\author{
\IEEEauthorblockN{Arman Rizwan Khan}
\IEEEauthorblockA{\textit{Dept. of Computer Science and Engineering}\\Priyadarshini College of Engineering\\Nagpur, Maharashtra, India\\arman.rkhan24@gmail.com}
\and
\IEEEauthorblockN{Aditya Shakya}
\IEEEauthorblockA{\textit{Dept. of Computer Science and Engineering}\\Priyadarshini College of Engineering\\Nagpur, Maharashtra, India\\adityashakya2712@gmail.com}
\and
\IEEEauthorblockN{Divyani Dinkar Waikar}
\IEEEauthorblockA{\textit{Dept. of Computer Science and Engineering}\\Priyadarshini College of Engineering\\Nagpur, Maharashtra, India\\divyaniwaikar2005@gmail.com}
\and
\IEEEauthorblockN{Aryan Manoj Terha}
\IEEEauthorblockA{\textit{Dept. of Computer Science and Engineering}\\Priyadarshini College of Engineering\\Nagpur, Maharashtra, India\\aryanterha7@gmail.com}
\and
\IEEEauthorblockN{Shreya Kamlesh Prasad}
\IEEEauthorblockA{\textit{Dept. of Computer Science and Engineering}\\Priyadarshini College of Engineering\\Nagpur, Maharashtra, India\\shreyaprasad5367@gmail.com}
\and
\IEEEauthorblockN{Rajshri Pote}
\IEEEauthorblockA{\textit{Dept. of Computer Science and Engineering}\\Priyadarshini College of Engineering\\Nagpur, Maharashtra, India\\rajshri.pote@pcenagpur.edu.in}}
...
\centering
\begin{tikzpicture}
\begin{axis}[width=\columnwidth,height=5.3cm,ybar,bar width=7pt,ymin=0,ymax=112,ylabel={Full-message recovery (\%)},symbolic x coords={Discord,WA Doc,Telegram,X,Instagram,WA Image},xtick=data,x tick label style={rotate=35,anchor=east,font=\scriptsize},ytick={0,20,40,60,80,100},nodes near coords,nodes near coords style={font=\scriptsize},grid=major]
\addplot coordinates {(Discord,100) (WA Doc,100) (Telegram,80) (X,0) (Instagram,0) (WA Image,0)};
\end{axis}
\end{tikzpicture}
\caption{Exact recovery rate by delivery mode.}
\label{fig:platform}
\end{figure}
...
\centering
\begin{tikzpicture}
\begin{axis}[width=\columnwidth,height=5.8cm,ybar,bar width=2.7pt,ymin=0,ymax=112,ylabel={Full-message recovery (\%)},symbolic x coords={small (10 B),medium (100 B),large (near max)},xtick=data,x tick label style={font=\scriptsize},ytick={0,20,40,60,80,100},legend style={font=\tiny,at={(0.5,-0.25)},anchor=north,legend columns=3},grid=major]
\addplot coordinates {(small (10 B),100) (medium (100 B),100) (large (near max),100)};
\addplot coordinates {(small (10 B),100) (medium (100 B),100) (large (near max),100)};
\addplot coordinates {(small (10 B),93.3) (medium (100 B),93.3) (large (near max),53.3)};
\addplot coordinates {(small (10 B),0) (medium (100 B),0) (large (near max),0)};
\addplot coordinates {(small (10 B),0) (medium (100 B),0) (large (near max),0)};
\addplot coordinates {(small (10 B),0) (medium (100 B),0) (large (near max),0)};
\legend{Discord,WhatsApp Document,Telegram,Twitter / X,Instagram,WhatsApp Image}
\end{axis}
\end{tikzpicture}
\caption{Recovery rate grouped by recorded payload class.}
\label{fig:payload}
\end{figure}
...
\midrule
Discord attachment & 15 x 3 & 45/45 & success \\
WhatsApp document & 15 x 3 & 45/45 & success \\
Telegram sendPhoto & 15 x 3 & 36/45 & capacity / fixed-grid \\
X image post & 45 x 100 B & 0/45 & fixed-grid extraction \\
Instagram feed & 45 x 100 B & 0/45 & fixed-grid extraction \\
WhatsApp image & 45 x 100 B & 0/45 & fixed-grid extraction \\
\bottomrule
\end{tabularx}
\end{table}
